...
> **Note on this file:** Everything below the `--- REPOSITORY GUIDE ENDS ---` marker is the
> repository's original `CLAUDE.md` — a comprehensive **Claude Code Development Rules** ruleset
> that also serves as live training content. It is preserved verbatim. The section immediately
> below is an added repository overview.

## Repository Purpose
A comprehensive **training repository** demonstrating how to configure, use, and compare AI
development agents (**Claude Code** and **Kiro**) with **Model Context Protocol (MCP)** servers.
It is both a learning resource and a practical reference for security-first, AI-assisted
development across application development, Infrastructure as Code, and DevOps automation.

- **Remote:** `git@github.com:brightkeycloud-chad/ai-agents-and-mcp.git`
- **Tech stack:** Python 3.8+ / Flask, Terraform (HCL), AWS (S3, Lambda, Bedrock) via boto3;
  tooling includes pytest, Bandit, Pylint, Black, MyPy, tflint, checkov.
- **MCP servers used:** AWS Diagram, AWS Pricing, Terraform, documentation generation.

## Module Map (progressive curriculum)
```
ai-agents-and-mcp/
├── 01_claude_code_config/    # Claude Code instruction files (CLAUDE_big.md, terraform-CLAUDE.md)
├── 02_kiro_config/           # Kiro "steering" system, organized by domain (security/dev/IaC/...)
├── 03_use_solutions/         # legacy-app-vulnerable vs legacy-app-secure Flask app
│                             #   + SECURITY-COMPARISON.md (before/after hardening)
├── 04_mcp_tests/             # MCP demos: 01_diagrams, 02_cost_analysis, 03_simple_app
├── 07_agents_mcp_development/ # Custom agent skills (e.g., MTG_CARD_CREATOR_PROMPT.md)
├── 08_agents_mcp_devops/     # 01_terraform_agent: a Terraform "cloud engineer" agent + module
├── CLAUDE.md                 # THIS FILE (repo guide + Claude Code rules)
└── README.md                 # Full setup, usage, troubleshooting, and cleanup guide
```

### Demos & modules — purpose and services covered

**01_claude_code_config** — *Configuration demo.*
- **Purpose:** show how to instruct Claude Code via `CLAUDE_big.md` (general dev guidelines) and
  `terraform-CLAUDE.md` (multi-file Terraform awareness, tflint, checkov).
- **AWS services:** none directly (agent-configuration content).

**02_kiro_config** — *Configuration demo.*
- **Purpose:** Kiro "steering" files (auto-loaded from `~/.kiro/steering/`) organized by domain
  (security, development, IaC, general, api, monitoring, compliance, resilience, documentation)
  and rolled out in priority phases (Critical → High → Medium).
- **AWS services:** none directly; rules govern how AWS work is done (IAM least-privilege,
  Lambda, tagging, cost).

**03_use_solutions** — *Secure-coding before/after demo (flagship).*
- **Purpose:** an intentionally **vulnerable** Flask app (`legacy-app-vulnerable/`: SQL-injection,
  weak password hashing, insecure file handling) and its **secured** rewrite
  (`legacy-app-secure/`: Argon2 hashing, input validation/sanitization, pytest suite, Bandit scan)
  — see `SECURITY-COMPARISON.md`.
- **AWS services:** **S3** (file storage) accessed with **STS** temporary credentials under
  least-privilege **IAM** (vs. the insecure original).

**04_mcp_tests** — *MCP capability demos.*
- **01_diagrams/** (`diagram_only`, `knowledge_and_diagram`) — **Purpose:** prompt the agent to
  design a cloud-native **three-tier architecture** (Layer-7 load balancer with a secure listener,
  Node.js in auto-scaled Docker containers, highly-available PostgreSQL) and render it with the AWS
  official icon set. **AWS services depicted:** ELB/**ALB**, **ECS/Fargate**, **EC2 Auto Scaling**,
  **RDS** (PostgreSQL), **VPC**. **MCP:** AWS Diagram MCP (+ AWS Knowledge MCP in the
  `knowledge_and_diagram` variant).
  - **02_cost_analysis/** — `01_diagram_cost` estimates the monthly cost of an architecture
  diagram; `02_monthly_bill_analysis` builds a 3-month report (top-10 services, biggest
  variations, recurring charges). **AWS services:** **Cost Explorer**, **Pricing API**.
  **MCP:** AWS Pricing MCP, AWS Cost Explorer MCP.
  - **03_simple_app/** — **Purpose:** generate a Stable Diffusion **image-generation web app**
  (port 8080, local-GPU inference, model cached in `./model/`) while obeying the rules in
  `~/.kiro/steering/`. **AWS services:** none (local GPU app); demonstrates Kiro steering + MCP-
  guided front-end generation.

**07_agents_mcp_development** — *Custom agent-skill demo.*
- **Purpose:** build a domain-specific Claude Code skill (`MTG_CARD_CREATOR_PROMPT.md` + a custom
  skill under `.claude/`) — a Magic: The Gathering card creator.
- **AWS services:** none directly.

**08_agents_mcp_devops/01_terraform_agent** — *DevOps automation demo.*
- **Purpose:** a specialized Terraform "cloud engineer" agent
  (`terraform-cloud-engineer-agent.json`) with its own `skills/` and an example multi-file
  `terraform/` module, showing agent-driven IaC authoring with cost awareness.
- **AWS services:** whatever the example Terraform module provisions (general AWS IaC); workflow
  uses the Terraform MCP + AWS Pricing MCP.

## Working In This Repo
- The full **README.md** is the authoritative setup/usage/troubleshooting/cleanup reference.
- Python work uses a project-root virtualenv; Terraform demos use the standard
  `init → fmt → validate → plan → apply` flow and should be `terraform destroy`-ed afterward.
- This repo's own development conventions are the ruleset preserved below — follow it when
  contributing here.

--- REPOSITORY GUIDE ENDS ---
